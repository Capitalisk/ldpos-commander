...
class CmdInterface {
    ...
    try {
      ...
      for (let i = 0; i < commands.length; i++) {
        ...
        if (accumulator[currentValue] !== undefined) {
          ...
          if (typeof accumulator === 'function') return await accumulator();
        } else if (typeof accumulator[currentValue] === 'function')
          return await accumulator[currentValue]();
        else throw new Error('command invalid');
      }

      // await [].concat(cmd).reduce(async (accumulator, k) => {
      //   const acc = await accumulator;
      //   if (typeof acc === 'string') acc = kebabCaseToCamel(acc);
      //   if (typeof k === 'string') k = kebabCaseToCamel(k);

      //   if (typeof acc === 'function') return await acc();
      //   else if (typeof acc[k] === 'function') return await acc[k]();
      //   else if (typeof acc[k] === 'object') return Promise.resolve(acc[k]);
      //   else throw new Error('command invalid');
  ...
  successLog(successMsg) {
    ...
    if (!this.config.interactive) process.exit();
  }
