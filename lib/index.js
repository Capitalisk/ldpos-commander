...
class CmdInterface {
  ...
  getCommands() {
    return {
      ...
      tests: {
        updateResult: async () =>  {
          let result = await client.syncAllKeyIndexes()
          this.successLog(result)
          result = await client.syncKeyIndex('forging')
          this.successLog(result)
        },
        deep: {
          nesting: {
  ...
  async execCmd(cmd) {
    ...
    try {
      await [].concat(cmd).reduce((acc, k) => {
        if (typeof acc === 'string') acc = kebabCaseToCamel(acc)
        if (typeof k === 'string') k = kebabCaseToCamel(k);
        if (typeof acc === 'function') Promise.resolve(acc());
        else if (typeof acc[k] === 'function') Promise.resolve(acc[k]());
