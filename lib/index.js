...
class CmdInterface {
  constructor(config) {
    this.config = {
      ...
      networkSymbol: config.networkSymbol ? config.networkSymbol : 7001,
    };
  ...
  async getConfig() {
    ...
    if (await fs.pathExists(this.fullConfigPath)) {
      ...
      config = {
        ...
        networkSymbol: await this.getNetworkSymbol(),
      };

      if (await this.getSaveConfig())
        await fs.outputFile(
          this.fullConfigPath,
  ...
  async getHostname() {
    return (await this.promptInput('Server IP:')) || '34.227.22.98';
  }

  async getPort() {
    return (await this.promptInput('Port: (Default: 7001)')) || 7001;
  }

  async getNetworkSymbol() {
    return (
      (await this.promptInput('Network symbol: (Default: ldpos)')) || 'ldpos'
    );
  }
