...
class CmdInterface {
  ...
  async interactive() {
    const cmd = await promptInput('cmd >');
  ...
  async execCmd(cmd) {
    ...
    if (!sw.hasOwnProperty(cmd)) {
      this.invalidCommand(true);
      this.interactive();
      return;
    }

    await (sw[cmd] || sw.default)();
    this.interactive();
  }
  ...
  async getPassphase() {
    return (
      (await promptInput('Passphrase:', true)) ||
...
const createInterface = async (config) => {
  if (!config.hostname)
    if (await fs.pathExists(this.fullConfigPath)) {
      ...
      // prettier-ignore
      config = {
        hostname: await promptInput('Server IP:') || '34.227.22.98',
      ...
      const save = ['Y', 'y'].includes(
        await promptInput(`Save in your home dir? (Y/n)`)
      );
