...
class CmdInterface {
  ...
  async execCmd(cmd) {
    ...
    // await (this.commands[cmd] || this.commands.default)(true);
  }
  ...
  async command(argv) {
    ...
    try {
      ...
      if (this.commands[type]) {
        if (this.commands[type].hasOwnProperty(command)) {
          ...
          await (this.commands[type][command] || this.commands.default)({
            client,
            passphrase,
          });
        } else {
