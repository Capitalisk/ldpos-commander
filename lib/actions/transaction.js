...
const postMultisigTransfer = async function (client) {
  const minFees = client.getMinFees();
  ...
  const memberFees =
    minFees.minMultisigTransactionFeePerMember * preparedTxn.signatures.length;

  preparedTxn.fee &&
    (preparedTxn.fee = parseInt(preparedTxn.fee) + memberFees).toString();
