...
const signMultisigTransfer = async function (client) {
  ...
  const fileLocation = await this.promptInput(
    'File location (without filename):'
  );
...
const postMultisigTransfer = async function (client) {
  ...
  if (
    ...
    const dirPrompt = async () => {
      const dirPath = await this.promptInput(
        'Location directory of the signitures: '
      );
      try {
        const files = await fs.readdir(dirPath);
        for (let i = 0; i < files.length; i++) {
          const file = files[i];
          signatures.push(require(`${dirPath}/${file}`).signature);
        }
      } catch (e) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const { signature } = require(fileLocation);
