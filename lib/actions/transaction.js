...
const getClipboard = (string) => {
  exec(`echo "${string}" | /usr/bin/xclip`);
};
...
const createMultisigTransfer = async function (client) {
  ...
  getClipboard(JSON.stringify(txnInJson));
  console.log(
    'Copy the below object (done automatically if xclip is present)\n\n'
  );
...
const signMultisigTransfer = async function (client) {
  ...
  this.successLog(multisigTxnSignature, 'Multisig transaction signed:');
};
...
const postMultisigTransfer = async function (client) {
  ...
  const count = await this.promptInput('Count of signatures to be added:');

  const signatures = [];
  for (let i = 0; i < count; i++) {
    const signature = await this.promptInput('Member signature:');
    if (signature === '') {
      this.errorLog('No signature provided', null, true);
      i--;
      continue;
    }
