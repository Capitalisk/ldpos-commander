...
const transfer = async function (client) {
  ...
  const amount = await this.promptInput('Amount of the transfer:');
  if (typeof parseInt(amount) !== 'number') throw new Error('Invalid amount.');

  const fee =
    (await this.promptInput(`Fee (Default: ${DEFAULT_MIN_TRANSACTION_FEES.transfer})`)) || DEFAULT_MIN_TRANSACTION_FEES.transfer;
  const message =
...
const vote = async function (client) {
  ...
  const fee =
    (await this.promptInput(`Fee (Default: ${DEFAULT_MIN_TRANSACTION_FEES.vote})`)) || DEFAULT_MIN_TRANSACTION_FEES.vote;
  const message =
...
const unvote = async function (client) {
  ...
  const fee =
    (await this.promptInput(`Fee (Default: ${DEFAULT_MIN_TRANSACTION_FEES.unvote})`)) || DEFAULT_MIN_TRANSACTION_FEES.unvote;
  const message =
    (await this.promptInput('Message to send of the recipient:')) || '';
...
const registerMultisigWallet = async function (client) {
  ...
  const fee =
    (await this.promptInput(`Fee (Default: ${DEFAULT_MIN_TRANSACTION_FEES.registerMultisigWallet})`)) ||
...
const registerMultisigDetails = async function (client) {
  ...
  const message = await this.promptInput('Message (optional):');
  const fee =
    (await this.promptInput(`Fee (Default: ${DEFAULT_MIN_TRANSACTION_FEES.registerMultisigDetails})`)) ||
...
const registerSigDetails = async function (client) {
  ...
  const message = await this.promptInput('Message (optional):');
  const fee =
    (await this.promptInput(`Fee (Default: (Default: ${DEFAULT_MIN_TRANSACTION_FEES.registerSigDetails})`)) ||
...
const registerForgingDetails = async function (client) {
  ...
  const message = await this.promptInput('Message (optional):');
  const fee =
    (await this.promptInput(`Fee (Default: ${DEFAULT_MIN_TRANSACTION_FEES.registerForgingDetails})`)) ||
