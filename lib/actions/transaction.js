...
const { _decimalToInteger, _integerToDecimal } = require('../utils');
const { SIGNATURE_PATH } = require('../constants');
...
const signMultisigTransfer = async function (client) {
  ...
  const fileLocation =
    (await this.promptInput(
      `Location to store the signature without filename (Default: ${SIGNATURE_PATH}): `
    )) || SIGNATURE_PATH;

  const absolutePath = `${fileLocation}/signature_${preparedMultisigTxn.id}_${multisigTxnSignature.signerAddress}.json`;

  await fs.mkdirp(fileLocation);

  await fs.outputFile(
    absolutePath,
    JSON.stringify(multisigTxnSignature, null, 2)
  );
...
const postMultisigTransfer = async function (client) {
  ...
  if (
    ...
    const dirPrompt = async () => {
      const dirPath =
        (await this.promptInput(
          `Location directory of the signatures: (Default: ${SIGNATURE_PATH}): `
        )) || SIGNATURE_PATH;

      try {
        let files = await fs.readdir(dirPath);

        const exp = new RegExp('^signature.*' + preparedMultisigTxn.id + '*.');

        for (let i = 0; i < files.length; i++) {
          const file = files[i];
          if (exp.test(file)) {
            signatures.push(require(`${dirPath}/${file}`));
          }
        }
      } catch (e) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      signatures.push(require(fileLocation));
    }
  }

  const txn = await client.attachMultisigTransactionSignature(
    preparedMultisigTxn,
    signatures
  );

  await client.postTransaction(txn);

  delete txn.signatures;
  txn.amount && (txn.amount = _integerToDecimal(txn.amount));
  txn.fee && (txn.fee = _integerToDecimal(txn.fee));

  this.successLog(txn, 'Successful transaction:');
};
