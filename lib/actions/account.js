...
const generateWallet = async function (client) {
  const { address, passphrase } = await client.generateWallet();

  const seed = client.computeSeedFromPassphrase(passphrase);

  const sigPublicKey = await client.computePublicKeyFromSeed(
    seed,
    'sig',
    0
  );
  const multisigPublicKey = await client.computePublicKeyFromSeed(
    seed,
    'multisig',
    0
  );
  const forgingPublicKey = await client.computePublicKeyFromSeed(
    seed,
    'forging',
    0
  );
  ...
  this.successLog(details, 'Generated wallet and public keys:');
};
