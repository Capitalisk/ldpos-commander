...
const generatePassphrase = async function () {
  ...
  this.successLog(passphrase, 'Generated passphrase:');
};

const generate = async function (networkSymbol) {
  if (!networkSymbol) networkSymbol = await this.promptInput('Network Symbol:');

  const wallet = await generateWallet(networkSymbol);
  const publicKeys = await computePublicKeysFromPassphrase(
    networkSymbol,
    wallet.passphrase
  );
